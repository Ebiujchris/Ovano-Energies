...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({
    ...
    stockQuantity: '', lowStockThreshold: '', category: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      bustCache('/products');
      setForm({ name: '', buyingPrice: '', sellingPrice: '', stockQuantity: '', lowStockThreshold: '', category: '' });
      reload();
    } catch (err) {
  ...
  }), [products, search, stockFilter, activeCategory]);
          ...
          <div className="mt-4 space-y-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Solar Panels, Phones…"
                value={form.category}
            ...
            <div className="flex gap-2">
              {(['all', 'low', 'out'] as StockFilter[]).map((f) => (
                <button key={f} type="button" onClick={() => setStockFilter(f)}
                  ...
                  {f === 'all' && `All stock (${products.length})`}
                  {f === 'low' && `Low (${lowCount})`}
                  {f === 'out' && `Out (${outCount})`}
                </button>
              ))}
